...
| Browser/web/search/HTTP | Playwright-backed isolated browser session for public navigation/extraction/screenshots/downloads; configured search service; bounded HTTP retrieval | Authenticated browsing is an explicit capability connection; no personal cookie import or autonomous purchasing |
...
| Browser/search/HTTP | Playwright; configured maintained search API/client; Bun HTTP; inspect existing extraction/rate-limit behavior | Capability grants, evidence, isolated profiles, budgets |
